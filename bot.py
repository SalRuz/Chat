...
TRANSLATIONS = {
    'en': {
        ...
        'allow_buyout': "✅ Allow Buyout", 'no_buyout': "❌ No Buyout",
        'allow_partial_build_question': "Allow building without full color group?",
        'allow_partial_build': "✅ Allow", 'no_partial_build': "❌ No",
    ...
    'ru': {
        ...
        'allow_buyout_question': "Выкуп недвижимости у других игроков?",
        'allow_buyout': "✅ Да", 'no_buyout': "❌ Нет",
        'allow_partial_build_question': "Строить без полной группы цвета?",
        'allow_partial_build': "✅ Да", 'no_partial_build': "❌ Нет",
...
def init_db():
    ...
    c.execute(
        "CREATE TABLE IF NOT EXISTS player_messages(room_code TEXT,user_id INTEGER,message_id INTEGER,PRIMARY KEY(room_code,user_id))")
    conn.commit()
    conn.close()
...
def get_room_state(code):
    if code not in ROOM_STATES:
        ROOM_STATES[code] = {'event_log': [], 'in_cooldown': False, 'is_moving': False, 'emojis': {},
                              'trades': {}, 'turn_timer': None, 'trade_building': {}}
...
GROUP_COLORS_HEX = {"brown": "#8B4513", "lightblue": "#87CEEB", "pink": "#FF69B4", "orange": "#FFA500",
                     "red": "#FF3030", "yellow": "#FFD700", "green": "#2E8B57", "darkblue": "#1E3A8A"}

BOARD = [
    {"name": "START", "type": "go"},
    {"name": "Mediterranean", "type": "property", "group": "brown", "price": 60, "rent": [2, 10, 30, 90, 160, 250],
     "house": 50},
    {"name": "Chest", "type": "chest"},
    {"name": "Baltic Ave", "type": "property", "group": "brown", "price": 60, "rent": [4, 20, 60, 180, 320, 450],
     "house": 50},
    ...
    {"name": "JAIL", "type": "jail"},
    {"name": "St.Charles", "type": "property", "group": "pink", "price": 140, "rent": [10, 50, 150, 450, 625, 750],
     "house": 100},
    {"name": "Electric Co", "type": "property", "group": "utility", "price": 150, "rent": [4, 10]},
    {"name": "States Ave", "type": "property", "group": "pink", "price": 140, "rent": [10, 50, 150, 450, 625, 750],
     "house": 100},
    {"name": "Virginia Ave", "type": "property", "group": "pink", "price": 160, "rent": [12, 60, 180, 500, 700, 900],
     "house": 100},
    {"name": "Pennsylvania RR", "type": "property", "group": "station", "price": 200, "rent": [25, 50, 100, 200]},
    {"name": "St.James", "type": "property", "group": "orange", "price": 180, "rent": [14, 70, 200, 550, 750, 950],
     "house": 100},
    {"name": "Chest", "type": "chest"},
    {"name": "Tennessee", "type": "property", "group": "orange", "price": 180, "rent": [14, 70, 200, 550, 750, 950],
     "house": 100},
    ...
    {"name": "Marvin Gdns", "type": "property", "group": "yellow", "price": 280,
    ...
    {"name": "N.Carolina", "type": "property", "group": "green", "price": 300,
     "rent": [26, 130, 390, 900, 1100, 1275], "house": 200},
    {"name": "Chest", "type": "chest"},
    {"name": "Pennsylvania", "type": "property", "group": "green", "price": 320,
...
class Database:
    ...
    def create_room(room):
        ...
        try:
            conn.execute(
                "INSERT INTO rooms(code,creator_id,chat_id,max_players,password,current_turn,is_started,last_message_id,chance_deck,risk_deck,awaiting_buy,awaiting_buyout,language,player_ids,allow_buyout,turn_time,allow_partial_build) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (room.code, room.creator_id, room.chat_id, room.max_players, room.password, room.current_turn,
                 room.is_started, room.last_message_id, json.dumps(room.chance_deck), json.dumps(room.risk_deck),
                 room.awaiting_buy, room.awaiting_buyout, room.language, json.dumps(list(room.player_ids)),
                 room.allow_buyout, room.turn_time, room.allow_partial_build))
            conn.commit()
            return True
        ...
        try:
            ...
            for pr in c.fetchall():
                ...
                room.players[p['user_id']] = Player(
                    ...
                    is_active=bool(p['is_active']), doubles_count=p.get('doubles_count', 0)
                )
    ...
    def update_room(room):
        ...
        try:
            conn.execute(
                "UPDATE rooms SET current_turn=?,is_started=?,last_message_id=?,chance_deck=?,risk_deck=?,awaiting_buy=?,awaiting_buyout=?,language=?,player_ids=?,allow_buyout=?,turn_time=?,allow_partial_build=? WHERE code=?",
                (room.current_turn, room.is_started, room.last_message_id, json.dumps(room.chance_deck),
                 json.dumps(room.risk_deck), room.awaiting_buy, room.awaiting_buyout, room.language,
                 json.dumps(list(room.player_ids)), room.allow_buyout, room.turn_time,
                 room.allow_partial_build, room.code))
            conn.commit()
        except Exception as e:
            logging.error(f"update_room:{e}")
        finally:
            conn.close()
    ...
    def add_player(code, player):
        ...
        try:
            conn.execute(
                "INSERT INTO players(room_code,user_id,name,money,position,color,is_bot,in_jail,jail_turns,is_active,doubles_count) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                (code, player.user_id, player.name, player.money, player.position, player.color, player.is_bot,
    ...
    def update_player(code, player):
        ...
        try:
            conn.execute(
                "UPDATE players SET money=?,position=?,in_jail=?,jail_turns=?,is_active=?,doubles_count=? WHERE room_code=? AND user_id=?",
                (player.money, player.position, player.in_jail, player.jail_turns, player.is_active,
                 player.doubles_count, code, player.user_id))
            conn.commit()
        except Exception as e:
            logging.error(f"update_player:{e}")
        finally:
            conn.close()
    ...
    def get_all_rooms():
        ...
        try:
            ...
            for row in c.fetchall():
                ...
                rooms.append({'code': row[0], 'has_password': bool(row[1]), 'max_players': row[2],
...
def get_props(room, pid):
    return [(i, BOARD[i], h) for i, (oid, h) in room.ownership.items() if
            oid == pid and BOARD[i]["type"] == "property"]


def count_stations(room, pid):
    return sum(1 for i, c in enumerate(BOARD) if
               c.get("group") == "station" and i in room.ownership and room.ownership[i][0] == pid)


def count_utils(room, pid):
    return sum(1 for i, c in enumerate(BOARD) if
               c.get("group") == "utility" and i in room.ownership and room.ownership[i][0] == pid)
...
def three_streets(room, pid):
    return sum(
        1 for g in ["brown", "lightblue", "pink", "orange", "red", "yellow", "green", "darkblue"] if
        owns_full_group(room, pid, g)) >= 3
...
def can_build(room, pid, idx):
    ...
    if oid != pid or h >= 5:
        return False, 0
    # Check if full group is required
...
async def _turn_timer_task(room_code):
    ...
    if room.can_roll and room.awaiting_buy is None:
        ...
        if cur.doubles_count > 0:
            ...
            await do_delay(room.code, 2, 'turn_end_countdown')
            await end_turn(room)
...
async def send_board(room, force=False, timer_text=None):
    ...
    for uid in room.player_ids:
        ...
        if cur and room.is_started and uid == cur.user_id and not st['is_moving'] and not st['in_cooldown']:
            if room.can_roll and room.awaiting_buy is None:
                kb_buttons.append(
                    [InlineKeyboardButton(text=t('roll_dice', room.language), callback_data=f"roll_{room.code}")])
                # Build button for any owned property at current position
                idx = cur.position
                ...
                if cb_ok:
                    ...
                    kb_buttons.append(
                        [InlineKeyboardButton(text=lb, callback_data=f"build_{room.code}_{idx}")])
                if not cur.is_bot:
                    kb_buttons.append(
                        [InlineKeyboardButton(text=t('trade', room.language), callback_data=f"trade_{room.code}")])
            if room.awaiting_buy is not None:
                ...
                kb_buttons.append(
                    [InlineKeyboardButton(text=t('skip', room.language), callback_data=f"skipbuy_{room.code}")])
            if cur.in_jail and room.awaiting_buy is None:
                kb_buttons.append(
                    [InlineKeyboardButton(text=t('pay_jail', room.language), callback_data=f"payjail_{room.code}")])
        ...
        if uid in trades:
            kb_buttons.append([
                InlineKeyboardButton(text=t('trade_confirm', room.language),
                                     callback_data=f"tradeaccept_{room.code}_{uid}"),
                InlineKeyboardButton(text=t('trade_reject', room.language),
            ...
            if uid in room.player_message_ids:
                try:
                    await bot.edit_message_media(
                        chat_id=uid, message_id=room.player_message_ids[uid],
                    ...
                    if "not modified" in es:
                        ...
                        try:
                            msg = await bot.send_photo(chat_id=uid,
                ...
                msg = await bot.send_photo(chat_id=uid, photo=BufferedInputFile(img, filename="b.png"),
                                           caption=text, parse_mode="Markdown", reply_markup=kb)
                room.player_message_ids[uid] = msg.message_id
                db.set_player_message_id(room.code, uid, msg.message_id)
                LAST_EDIT[uid] = time.time()
        except Exception as e:
            logging.error(f"send:{uid}:{e}")
...
async def cb_browse(cb: CallbackQuery):
    ...
    if not rooms:
        await cb.message.edit_text(t('no_rooms', lang), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=t('create_room', lang), callback_data="create_room")],
    ...
    for r in rooms:
        ...
        if not r['has_password']:
            kbb.append(
                [InlineKeyboardButton(text=t('join', lang, r['code']), callback_data=f"quickjoin_{r['code']}")])
...
async def cb_back(cb: CallbackQuery, state: FSMContext):
    await state.clear()
    lang = db.get_user_language(cb.from_user.id)
    await cb.message.edit_text(t('welcome', lang), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t('create_room', lang), callback_data="create_room")],
        [InlineKeyboardButton(text=t('join_by_code', lang), callback_data="join_room")],
        [InlineKeyboardButton(text=t('browse_rooms', lang), callback_data="browse_rooms")]]), parse_mode="Markdown")


@router.callback_query(F.data == "create_room")
async def cb_create(cb: CallbackQuery, state: FSMContext):
    lang = db.get_user_language(cb.from_user.id)
    await cb.message.edit_text(t('choose_max_players', lang), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_max(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(t('password_question', lang, mp), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_nopass(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(t('allow_buyout_question', lang), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def msg_pass(msg: Message, state: FSMContext):
    ...
    await msg.answer(t('allow_buyout_question', lang), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t('allow_buyout', lang), callback_data="buyout_yes")],
        [InlineKeyboardButton(text=t('no_buyout', lang), callback_data="buyout_no")]]))
    await state.set_state(CreateRoom.wait_buyout)
...
async def cb_partial_build(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(t('choose_turn_time', lang), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t('seconds_15', lang), callback_data="time_15")],
        [InlineKeyboardButton(text=t('seconds_30', lang), callback_data="time_30")],
        [InlineKeyboardButton(text=t('seconds_60', lang), callback_data="time_60")]]))
...
async def cb_time(cb: CallbackQuery, state: FSMContext):
    ...
    lang = db.get_user_language(cb.from_user.id)
    code = generate_code()
    room = Room(code=code, creator_id=cb.from_user.id, chat_id=cb.from_user.id,
...
async def show_lobby(m, room):
    ...
    text = t('room_info', room.language, room.code, room.password or t('none', room.language),
...
async def cb_addbot(cb: CallbackQuery):
    ...
    if not db.add_player(code, p):
        await cb.answer("Full", show_alert=True)
        return
    room.players[bid] = p
...
async def cb_join_menu(cb: CallbackQuery, state: FSMContext):
    await cb.message.edit_text(t('enter_room_code', db.get_user_language(cb.from_user.id)))
...
async def handle_trade_money_input(message: Message, state: FSMContext):
    ...
    if phase == 'want':
        ...
        if cur:
            for pi, pc, ph in get_props(room, cur.user_id):
                btns.append([InlineKeyboardButton(text=f"🏠 {pc['name']}",
                                                  callback_data=f"tgive_{code}_prop_{pi}")])
        btns.append(
            [InlineKeyboardButton(text=t('trade_money', room.language), callback_data=f"tgive_{code}_money")])
        btns.append(
            [InlineKeyboardButton(text=t('trade_cancel', room.language), callback_data=f"tcancel_{code}")])
...
# ─── EMOJI HANDLER (must be LAST message handler, no state) ───
@router.message(~StateFilter(CreateRoom.wait_password, JoinRoom.wait_code, JoinRoom.wait_password, TradeMoneyInput.waiting, CreateRoom.wait_max, CreateRoom.wait_buyout, CreateRoom.wait_partial_build, CreateRoom.wait_turn_time))
...
async def cb_roll(cb: CallbackQuery):
    ...
    room = db.get_room(code)
    if not room or not room.is_started:
        await cb.answer("No", show_alert=True)
        return
    cur = room.current_player()
...
async def do_roll(room):
    ...
    if cur.in_jail:
        ...
        if d1 == d2:
            ...
            await send_board(room)
            await do_delay(room.code, 2, 'moving_countdown')
            room = db.get_room(room.code)
            if not room:
                return
                        ...
                        if room:
                            ...
                            if p:
                                ...
                                if c2:
                                    ...
                                    room.add_event(t('hotel_built' if hh + 1 >= 5 else 'house_built',
                                                     room.language, p.name, cell['name'], hh, hh + 1))
                                    await send_board(room)
                    await maybe_bot(room)
            else:
                room.can_roll = False
            ...
            if not player.is_bot:
                ...
                if room and room.awaiting_buy == idx:
                    ...
                    if p:
                        ...
                        db.update_player(room.code, p)
                        room.add_event(
                            t('property_bought', room.language, p.name, cell['name'], cell['price']))
...
async def maybe_bot(room):
    ...
    if not room or not room.is_started or not room.can_roll:
        return
    if get_room_state(room.code)['in_cooldown']:
        return
    cur = room.current_player()
...
async def cb_build(cb: CallbackQuery):
    ...
    await cb.answer()
    room.add_event(
        t('hotel_built' if h + 1 >= 5 else 'house_built', room.language, cur.name, cell['name'], h, h + 1))
...
async def cb_trade_start(cb: CallbackQuery):
    ...
    for p in room.active_players():
        if p.user_id != cur.user_id and not p.is_bot:
            btns.append([InlineKeyboardButton(text=f"👤 {p.name} (${p.money})",
...
async def cb_trade_player(cb: CallbackQuery):
    ...
    for pi, pc, ph in get_props(room, target_id):
        btns.append([InlineKeyboardButton(text=f"🏠 {pc['name']} ({ph}🏠)",
                                          callback_data=f"twant_{code}_prop_{pi}")])
    btns.append(
        [InlineKeyboardButton(text=t('trade_money', room.language), callback_data=f"twant_{code}_money")])
    btns.append(
        [InlineKeyboardButton(text=t('trade_cancel', room.language), callback_data=f"tcancel_{code}")])
    try:
...
async def cb_trade_want(cb: CallbackQuery, state: FSMContext):
    ...
    if not tb:
        await cb.answer("No trade", show_alert=True)
        return
